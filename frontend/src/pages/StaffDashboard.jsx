...
import { getStaffOrders, completeStaffOrder } from '../services/ordersApi';
...
const StaffDashboard = () => {
    ...
    return (
        <div className="font-body-md flex flex-col p-8">
            ...
            <h2 className="font-headline-sm text-secondary mb-4">Completed Orders ({completedOrders.length})</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {completedOrders.map(order => (
                    <div key={order.id} className="bg-surface-container-low rounded-xl p-4 border border-outline-variant/30 opacity-70">
                        ...
                        <span className="text-secondary font-bold text-sm">Completed</span>
                    </div>
                ))}
                {completedOrders.length === 0 && <div className="text-on-surface-variant">No completed orders.</div>}
            </div>
        </div>
    );
};
