...
const ProfileDropdown = () => {
    ...
    // Check if user is vendor via metadata or if currently on a vendor route
    const isOwner = user?.publicMetadata?.role === 'vendor' || location.pathname.startsWith('/vendor'); 
    ...
    return (
        <div className="relative" ref={dropdownRef}>
            ...
            <div 
                ...
                className="w-10 h-10 rounded-full bg-surface-container-high border border-outline-variant/30 flex items-center justify-center cursor-pointer shadow-[0_2px_8px_rgba(0,0,0,0.2)] hover:bg-surface-variant transition-colors active:scale-95 overflow-hidden"
            >
                {user?.imageUrl ? (
            ...
            {isOpen && (
                <div className="absolute right-0 top-14 w-[280px] z-50 bg-[#1c1c1e] rounded-2xl shadow-[0_8px_32px_rgba(0,0,0,0.5)] border border-outline-variant/20 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                    ...
                    <div className="p-4 border-b border-outline-variant/10 flex items-center gap-3 bg-[#1a1a1c]">
                        <div className="w-10 h-10 rounded-full bg-primary-container flex items-center justify-center flex-shrink-0 shadow-inner overflow-hidden">
                            {user?.imageUrl ? (
                                <img src={user.imageUrl} alt="Profile" className="w-full h-full object-cover" />
                            ) : (
                                <span className="font-headline-sm text-on-primary-container font-bold">
                                    {user?.firstName?.charAt(0) || 'U'}
                                </span>
                            )}
                        </div>
                        <div className="flex flex-col">
                            <span className="font-label-md font-bold text-white">
                                {user ? `${user.firstName || ''} ${user.lastName || ''}`.trim() || 'User' : 'Guest User'}
                            </span>
                        </div>
                    </div>
                    ...
                    <div className="p-2 flex flex-col gap-1">
                        <button onClick={() => handleAction('profile')} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-[#2c2c2e] text-white/80 hover:text-white transition-all group cursor-pointer active:scale-95">
                            ...
                            <span className="font-label-md">Personal Profile</span>
                        </button>
                        
                        {isOwner && (
                            <button onClick={() => handleAction('canteen')} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-[#2c2c2e] text-white/80 hover:text-white transition-all group cursor-pointer active:scale-95">
                                <span className="material-symbols-outlined text-[20px] text-white/50 group-hover:text-white transition-colors" style={{ fontVariationSettings: "'wght' 300" }}>storefront</span>
